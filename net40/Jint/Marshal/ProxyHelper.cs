...
internal class ProxyHelper
{
  ...
  public JsMethodImpl WrapMethod(MethodInfo info, bool passGlobal)
  {
    ...
    foreach (ParameterInfo parameterInfo in linkedList1)
    {
      ...
      ilGenerator.Emit(OpCodes.Ble, label3);
      ilGenerator.Emit(OpCodes.Ldarg_2);
      ilGenerator.Emit(OpCodes.Ldc_I4, index);
  ...
  public ConstructorImpl WrapConstructor(ConstructorInfo info, bool passGlobal)
  {
    ...
    DynamicMethod dynamicMethod = new DynamicMethod("clrConstructor", typeof (object), new Type[2]
    {
      typeof (IGlobal),
      typeof (JsInstance[])
    }, this.GetType());
  ...
  private struct MarshalledParameter(LocalBuilder temp, int index)
  {
    public LocalBuilder tempLocal = temp;
